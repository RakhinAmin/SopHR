...
class PdfProcessor:
    ...
    SECTION_START = "Class 5 A USD Series 1"  
    SECTION_END = "SHAREHOLDER SUMMARY"  
    # Regular expression patterns for beginning shares, beginning value, net capital activity (shares), net capital activity, NAV per share, ending shares, ending value
    BEGINNING_SHARES_PATTERN = r"\d{1,3}(?:\.\d+)?"
    BEGINNING_VALUE_PATTERN = r"USD \d{1,3}(?:,\d{3})*,\d{2}"
    NET_CAPITAL_ACTIVITY_SHARES_PATTERN = r"\d{1,3}(?:\.\d+)?"
    NET_CAPITAL_ACTIVITY_PATTERN = r"USD \d{1,3}(?:,\d{3})*,\d{2}|USD 0"
    NAV_PER_SHARE_PATTERN = r"USD \d{1,3}(?:,\d{3})*\.\d{2}"
    ENDING_SHARES_PATTERN = r"\d{1,3}(?:\.\d+)?"
    ENDING_VALUE_PATTERN = r"USD \d{1,3}(?:,\d{3})*,\d{2}"
    ...
    def _parse_section(self, section):
        # Search for beginning shares, beginning value, net capital activity (shares), net capital activity, NAV per share, ending shares, ending value patterns in the section text
        beginning_shares = re.search(self.BEGINNING_SHARES_PATTERN, section)  
        beginning_value = re.search(self.BEGINNING_VALUE_PATTERN, section)  
        net_capital_activity_shares = re.search(self.NET_CAPITAL_ACTIVITY_SHARES_PATTERN, section)  
        net_capital_activity = re.search(self.NET_CAPITAL_ACTIVITY_PATTERN, section)  
        nav_per_share = re.search(self.NAV_PER_SHARE_PATTERN, section)  
        ending_shares = re.search(self.ENDING_SHARES_PATTERN, section)  
        ending_value = re.search(self.ENDING_VALUE_PATTERN, section)  

        # Check if all patterns were matched
        if all((beginning_shares, beginning_value, net_capital_activity_shares, net_capital_activity, nav_per_share, ending_shares, ending_value)):
            # Extract and return the matched values along with headers
            return (
                ["Beginning Shares", "Beginning Value", "Net Capital Activity (Shares)", "Net Capital Activity", "NAV Per Share", "Ending Shares", "Ending Value"],
                [g.group(0).strip() for g in (beginning_shares, beginning_value, net_capital_activity_shares, net_capital_activity, nav_per_share, ending_shares, ending_value)]
            )
...
class ExcelHandler:
    ...
    DEFAULT_FILENAME = 'Alphadyne_Cleaned_Data.xlsx'  
    SHEET_NAME = 'Alphadyne'  
...
if __name__ == "__main__":
    ...
    pdf_path = r'C:\Users\Rakhin.Amin\Documents\PythonPDFExtraction\Alphadyne\Alphadyne - September 2024 (002).pdf'  
