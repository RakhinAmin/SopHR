...
def setup_logging():
    ...
    file_handler = logging.FileHandler('categorizer.log', encoding='utf-8')  # Log file
...
class TransactionCategorizer:
    ...
    def __init__(self, config: Config):
        ...
        self._match_cache = {}  # Placeholder; lru_cache used elsewhere
...
def main():
    """Main entry point"""
    config = Config()  # Set values before execution
    categorizer = TransactionCategorizer(config)
    success = categorizer.run_categorization()
    sys.exit(0 if success else 1)  # Exit based on success
